...
function CompensationListTable(props) {
  const [
    recordIdToDestroy,
          ...
          <thead className="thead">
            <tr>
              <TableColumnHeader className="th-checkbox">
                ...
                <TableColumnHeader
                  label={i18n(
                    'entities.compensation.fields.person',
                  )}
                />
                <TableColumnHeader
                  onSort={doChangeSort}
                  ...
                  name={'type'}
                  label={i18n(
                    'entities.compensation.fields.type',
                  )}
                />
                <TableColumnHeader
                  onSort={doChangeSort}
                  ...
                  label={i18n(
                    'entities.compensation.fields.workAvailability',
                  )}
                  align="right"
                />
                <TableColumnHeader
                  ...
                  name={'monetary'}
                  label={i18n(
                    'entities.compensation.fields.monetary',
                  )}
                  align="right"
                />
                <TableColumnHeader
                  ...
                  name={'paidTimeOff'}
                  label={i18n(
                    'entities.compensation.fields.paidTimeOff',
                  )}
                  align="right"
                />
                <TableColumnHeader
                  ...
                  name={'effectiveDate'}
                  label={i18n(
                    'entities.compensation.fields.effectiveDate',
                  )}
                />
              ...
              rows.map((row) => (
                <tr key={row.id}>
                  ...
                  <td>
                    {row.type
                      ? i18n(
                          `entities.compensation.enumerators.type.${row.type}`,
                        )
                      : null}
                  </td>
                  <td style={{ textAlign: 'right' }}>{row.workAvailability}</td>
                  <td style={{ textAlign: 'right' }}>
                    {row.monetary}
                  </td>
                  <td style={{ textAlign: 'right' }}>{row.paidTimeOff}</td>
                  <td>{row.effectiveDate}</td>
                  <td className="td-actions">
                    <Link
                      className="btn btn-link"
                      to={`/compensation/${row.id}`}
                    >
                      {i18n('common.view')}
                    </Link>
                    {hasPermissionToEdit && (
...
export default CompensationListTable;
